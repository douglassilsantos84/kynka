...
class ProcurementService:
    ...
    def for_demand(self, demand_id: int) -> PurchaseList:
        ...
        if demand is None:
            raise DemandNotFoundError(
                f"Demanda não encontrada: {demand_id}."
            )
    ...
    def consolidated(
        ...
        if demand_ids is None:
            ...
            for value in demand_ids:
                ...
                demand = self._demand_repository.get_demand(
                    demand_id
                )
        ...
        for demand in demands:
            requirements = (
                self._demand_repository
                .list_requirements(demand.id)
            )

            for requirement in requirements:
                material = (
                    self._inventory_repository
                    .get_by_code(requirement.material_code)
                )
    ...
    def _find_demand_in_text(
        ...
        # Primeiro: código exato da demanda
        for demand in demands:
        ...
        # Finalmente: combinação das palavras significativas
        candidates = []
    ...
    def try_answer(self, text: str):
        """
        Responde exclusivamente perguntas relacionadas à necessidade
        e à lista de compras.

        Perguntas sobre fornecedor, preço, cotação ou decisão de onde
        comprar pertencem ao SupplierService e não devem ser capturadas
        por este serviço.
        """
        ...
        # ----------------------------------------------------
        # Intenções reservadas para SupplierService
        # ----------------------------------------------------

        supplier_intelligence_terms = (
            "fornecedor",
            "fornecedores",
            "quem vende",
            "onde comprar",
        ...
        if any(
            ...
            for term in supplier_intelligence_terms
        ):
            return None

        # ----------------------------------------------------
        # Intenções próprias da lista de compras
        # ----------------------------------------------------
        ...
        purchase_terms = (
            ...
            "quais materiais preciso comprar",
            "quais materiais faltam",
            "materiais faltam",
            "materiais estao faltando",
            "material falta",
            "material faltando",
            "falta comprar",
            "faltando",
        ...
        if any(
            ...
            return (
                purchase_list.analysis,
                purchase_list,
            )

        demand = self._find_demand_in_text(
            text,
            demands,
        )

        if demand is not None:
            purchase_list = self.for_demand(
                demand.id
            )
    ...
    def _analysis(
        ...
        if not codes:
            return (
                "Não há projetos selecionados "
                "para calcular a lista de compras."
            )

        scope = (
            codes[0]
            if len(codes) == 1
            else f"{len(codes)} projetos selecionados"
        )

        if not items:
            return (
                f"{scope}: não há necessidade de compra "
                "com os dados atuais. "
                "O estoque mínimo e as reservas existentes "
                "foram preservados."
            )

        lines = [
            (
                f"{scope}: {len(items)} de {total} "
                "material(is) precisam de compra."
            )
        ]
        ...
        for item in items[:8]:
            lines.append(
                ...
                f"{item.material_name}: comprar "
                f"{item.quantity_to_buy:g} "
                f"{item.unit}."
            )

        if len(items) > 8:
            lines.append(
                f"Há mais {len(items) - 8} "
                "material(is) na lista."
            )

        lines.append(
            "O cálculo preserva o estoque mínimo "
            "e desconta todas as reservas existentes."
        )
